...
export const OrderCard: React.FC<OrderCardProps> = ({ order }) => {
  ...
  return (
    <div className="rounded-3xl border border-[#C9AE8B]/40 bg-[#FAF4EB]/90 p-5 shadow-xs backdrop-blur-sm transition-all hover:shadow-md space-y-3">
      ...
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-[#725039] font-medium">{timeAgo}</span>
          </div>
          <h3 className="mt-1 font-serif text-lg font-bold tracking-tight text-[#241F1C]">
            {copy.title}
          </h3>
        </div>
      ...
      {canEdit ? (
        <div className="flex items-center justify-between rounded-2xl border border-[#F2C84B] bg-[#F2C84B]/15 p-3 text-xs">
          <div className="text-[#725039]">
            <span className="font-bold">Pending Confirmation</span>
            <p className="text-[11px] text-[#725039]/80">
        ...
        <div className="rounded-xl border border-[#C9AE8B]/30 bg-[#F3E7D3]/60 p-2.5 text-[11px] text-[#725039] font-serif italic text-center">
          Order confirmed. Preparation has started and editing is locked.
        </div>
      ) : null}

      <div className="flex items-center justify-between">
        <p className="font-serif text-xs leading-relaxed text-[#725039]">
          {copy.subtitle}
        </p>
        ...
        {(order.status === "PENDING_CONFIRMATION" ||
          ...
          order.etaMinMinutes &&
          order.etaMaxMinutes && (
            <div className="flex-shrink-0 ml-2 flex items-center gap-1 rounded-full bg-[#F2C84B]/20 border border-[#C9AE8B] px-2.5 py-1 text-[11px] font-mono font-bold text-[#725039]">
              <Clock className="h-3 w-3 text-[#B72E35]" />
              <span>
      ...
      <div className="mt-4 border-t border-[#C9AE8B]/30 pt-2">
        <OrderStatusProgress status={order.status} />
      </div>

      {/* Items Toggle & Breakdown */}
      <div className="mt-3 border-t border-[#C9AE8B]/30 pt-3">
        <button
          onClick={() => setShowItems((v) => !v)}
          className="flex w-full items-center justify-between text-xs font-serif font-bold text-[#725039] transition hover:text-[#241F1C]"
        >
          <span>
        ...
        {showItems && (
          <div className="mt-3 divide-y divide-[#C9AE8B]/20 rounded-2xl bg-[#F3E7D3]/50 p-3.5 text-xs">
            {order.items.map((item) => (
              <div key={item.id} className="flex items-center justify-between py-2">
                <div className="flex items-center gap-2">
                  <span className="font-mono font-bold text-[#725039]">{item.qty}x</span>
                  <span className="font-serif font-medium text-[#241F1C]">
                    {item.name.toLowerCase()}
                  </span>
                </div>
                <span className="font-mono font-semibold text-[#241F1C]">
                  ₹{Math.round(item.lineSubtotal / 100)}
                </span>
              </div>
            ))}

            <div className="flex items-center justify-between pt-2.5 font-bold text-[#241F1C] border-t border-[#C9AE8B]/30">
              <span className="font-serif">Total</span>
              <span className="font-mono text-sm text-[#B72E35]">
                ₹{totalRupees}
              </span>
            </div>
          </div>
